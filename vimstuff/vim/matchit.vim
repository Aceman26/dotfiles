...
aug Matchit
  ...
  " IDL: added by Marshall Perrin <mperrin@astro.berkeley.edu>
  " should be: - case insensitive
"    \ '\<pro\>:\<end\>,'.
  au FileType idlang if !exists("b:match_words") |
    \ let b:match_ignorecase=0 | let b:match_words =
    \ '(:),[:],{:},'.
    \ '\<case\>:\<endcase\>,'.
    \ '\<while\>:\<endwhile\>,'.
  ...
  " TCL: Added by Marshall Perrin
  au FileType tcl if !exists("b:match_words") |
    \ let b:match_ignorecase=0 | let b:match_skip = 'r:#' | let b:match_words =
    \ '{:},(:),[:]'
    \ | endif
aug END
